...
namespace ModComprobantes
{
    public partial class frmCompContListaGLB01 :  frmPlantilla, IReLocalizable
    {
            ...
            try
            {
                ...
                for (int i = 0; i < this.radGridViewComprobantes.SelectedRows.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        //if (autorizadoAprobar)
                        if (autorizadoAprobar && this.ValidarComprobanteCuadra(i)==true)
                        {
                            //Ocultar los grids de Visor del Historial y de Acciones si están visibles y agrandar el grid de comprobantes
                            this.OcultarGridHistorial_Acciones();
                        ...
                        //Validar Cuadre
                        bool comprobanteCuadra = ValidarComprobanteCuadra(i);
                        
                        if (!comprobanteCuadra) RadMessageBox.Show(this.LP.GetText("lblErrUserAprobCompDesc", "No es posible aprobar un Comprobante descuadrado ") + tipoCompAct, this.LP.GetText("errValTitulo", "Error"));   //Falta traducir;
                    }
                }
        ...
        private bool ValidarComprobanteCuadra(int i)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < this.radGridViewComprobantes.SelectedRows.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (autorizadoAprobar && autorizadoContabilizar)
                        {
                            if (estado != "A")
                            {
                                //Ocultar los grids de Visor del Historial y de Acciones si están visibles y agrandar el grid de comprobantes
                        ...
                        else
                        {
                            ...
                            else RadMessageBox.Show(this.LP.GetText("lblErrUserNoAutTipoCompCont", "Usuario no autorizado a contabilizar comprobantes de tipo ") + tipoCompAct, this.LP.GetText("errValTitulo", "Error"));   //Falta traducir
                        }
                    }
                }
        ...
        private void RadButtonBuscar_Click(object sender, EventArgs e)
        {
            ...
            if (this.FormValid(sap))
            {
                ...
                // string query = "select * from " + GlobalVar.PrefijoTablaCG + "GLI03 "; JL
                string query = "select * from " + GlobalVar.PrefijoTablaCG + "GLI03 " + "inner join " + GlobalVar.PrefijoTablaCG + "GLT06 " +
                    "on TICOIC =TIVOTV ";
