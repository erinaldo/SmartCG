...
namespace OutlookStyleControls
{
    ...
    public interface IOutlookGridGroup : IComparable, ICloneable
    {
        ...
        /// <summary>
        /// specifies the label of number of items that are part of the current group
        /// this value is automatically filled each time the grid is re-drawn
        /// e.g. after sorting the grid.
        /// </summary>
    ...
    public class OutlookgGridDefaultGroup : IOutlookGridGroup
    {
        ...
        public OutlookgGridDefaultGroup()
        {
            ...
            if (itemsName == null) itemsName = "items";
        }
        ...
        {
            get {
                string item = "1 " + itemName;
                string items = " " + itemsName;

                if (column == null)
                    return string.Format("Unbound group: {0} ({1})", Value.ToString(), itemCount == 1 ? item : itemCount.ToString() + items);
                else
                    return string.Format("{0}: {1} ({2})", column.HeaderText, Value.ToString(), itemCount == 1 ? item : itemCount.ToString() + items); 
                }
            set { text = value; }
        }
    ...
    public class OutlookGridAlphabeticGroup : OutlookgGridDefaultGroup
    {
        ...
        public override string Text
        {
            get
            {
                ...
                return string.Format("Alphabetic: {1} ({2})", column.HeaderText, Value.ToString(), itemCount == 1 ? item : itemCount.ToString() + items);
            }
            set { text = value; }
        }
