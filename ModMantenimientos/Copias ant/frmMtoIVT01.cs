    ...
    public partial class frmMtoIVT01 : frmPlantilla, IReLocalizable
    {
        ...
        */
        ...
        private void FrmMtoIVT01_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            try
            {
                    ...
                    )
                {
                    ...
                    {                       //jl
                        string mensaje = this.LP.GetText("confGuardarCambio", "¿Desea guardar los cambios efectuados?");
            ...
            try
            {
                ...
                for (int i = 0; i < this.radGridViewTiposIVA.Rows.Count; i++)
                {
                    ...
                    if (validarFilasTiposIVA)
                    {
                        ...
                        */
                        if (this.radGridViewTiposIVA.Rows[i].Cells["PorcRecEquiv"].Value == null)
                        {
                            mensaje = " Porcentaje de recargo de equivalencia no puede estar en blanco.";
                            if (errorFila == "") errorFila += " - Tipos de IVA Fila " + (i + 1) + ": " + mensaje;      //Falta traducir
                            else errorFila += mensaje;
                            this.radGridViewTiposIVA.CurrentRow = this.radGridViewTiposIVA.Rows[i];
                            this.radGridViewTiposIVA.CurrentColumn = this.radGridViewTiposIVA.Columns["PorcRecEquiv"];
                        }
        ...
        private string ValidarCodigoIVAContrap(string valor)
        {
            ...
            if (bCancelar == false && this.codigoIVACont == this.codigo) result = this.LP.GetText("lblCodIVAContErrCompExcep", "El código de IVA contrapartida no puede ser el mismo que se está definiendo");
            
            if (this.radButtonTextBoxCtaMayorContrap.Text.Trim() != "" && this.radButtonTextBoxCodIVAContrap.Text.Trim() != "")
                            result = this.LP.GetText("lblCodIVAContErrCompExcep", "No se pueden informar a la vez cuenta mayor y código IVA contrapartida");
