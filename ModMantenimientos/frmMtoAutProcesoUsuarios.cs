...
{
    public partial class frmMtoAutProcesoUsuarios : frmPlantilla, IReLocalizable
    {
        ...
        public frmMtoAutProcesoUsuarios()
        {
            InitializeComponent();
            ...
            this.radMultiColumnComboBoxProcesos.AutoSizeDropDownToBestFit = true;
        }
        ...
        private void frmMtoAutProcesoUsuarios_Load(object sender, EventArgs e)
        {
            Log.Info("INICIO Autorización para crear procesos");

            this.radMultiColumnComboBoxProcesos.Visible = true;
        ...
        {
            if (this.radMultiColumnComboBoxProcesos.SelectedValue != null)
            {
                Cursor.Current = Cursors.WaitCursor;
                this.codigo = this.radMultiColumnComboBoxProcesos.SelectedValue.ToString().Trim();
        ...
        private void frmMtoAutProcesoUsuarios_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void BuscarUsuariosAutorizados()
        {
            try
            {
                //Buscar todos los usuarios autorizados al procesos
                string query = "select * from " + GlobalVar.PrefijoTablaCG + "ATM08 ";
                query += "where CLELAG = '" + this.codigo + "' and GROPAG = '01' ";
                query += "order by IDUSAG";

                DataTable dtUsers = GlobalVar.ConexionCG.FillDataTable(query, GlobalVar.ConexionCG.GetConnectionValue);

                if (dtUsers != null && dtUsers.Rows != null && dtUsers.Rows.Count > 0)
                {
                    this.dtUsuariosAutAux = new DataTable();
                    this.dtUsuariosAutAux.Columns.Add("IDUSMO", typeof(string));
                    this.dtUsuariosAutAux.Columns.Add("NOMBMO", typeof(string));

                    DataRow row;
                    string userCod = "";
                    for (int i = 0; i < dtUsers.Rows.Count; i++)
                    {
                        row = dtUsuariosAutAux.NewRow();
                        userCod = dtUsers.Rows[i]["IDUSAG"].ToString();
                        row["IDUSMO"] = userCod;
                        row["NOMBMO"] = this.UsuarioNombre(userCod);
                        this.dtUsuariosAutAux.Rows.Add(row);
                    }
                }
            }
            catch (Exception ex) { Log.Error(Utiles.CreateExceptionString(ex)); }
        ...
        private void CargarInfoUsuarios()
        {
            try
            {
                ...
                {
                    //Busca los procesos autorizadas para el usuario indicado
                    ...
                    try
                    {
                        ...
                        foreach (DataRow row in this.dtUsuariosAux.Rows)
                        {
                            if (this.dtUsuariosAutAux.Rows != null && this.dtUsuariosAutAux.Rows.Count > 0)
                            {
                                ...
                                {
                                    if (rowUsuarioAutAux["IDUSMO"].ToString().Trim() == row["IDUSMO"].ToString().Trim())
                                    {
                                        //Insertar el proceso autorizada en el DataTable de procesos autorizados
        ...
        /// <summary>
        /// Elimina todos los usuarios que tenían el proceso anteriormente
        /// </summary>
        ...
        /// <summary>
        /// Graba todos los usuarios autorizados para el proceso
        /// </summary>
        private void GrabarAutUsuarios()
        {
            try
            {
                ...
                //Grabar para todos las usuarios el proceso autorizado
                if (this.autTodosUsuarios)
                {
                    if (this.dtUsuariosAux.Rows != null && this.dtUsuariosAux.Rows.Count > 1)
                    {
                        for (int i = 1; i < this.dtUsuariosAux.Rows.Count; i++)
                        {
                            ...
                            this.InsertarATM06(usuarioActual, this.codigo);
                        }
                    }
                }
                else
                {*/
                    for (int i = 0; i < this.radListControlUsuarioAutoriz.Items.Count; i++)
                    {
                        ...
                        this.InsertarATM08(this.codigo, usuarioActual);
                    }
                //}
        ...
        private void CargarListaProcesos()
        {
            try
            {
                RadMultiColumnComboBoxElement multiColumnComboElement = this.radMultiColumnComboBoxProcesos.MultiColumnComboBoxElement;
                ...
                column.HeaderText = "Proceso";
                multiColumnComboElement.Columns.Add(column);
                column = new GridViewTextBoxColumn("NOMBAA");
                column.HeaderText = "Nombre";
                multiColumnComboElement.Columns.Add(column);

                this.radMultiColumnComboBoxProcesos.MultiColumnComboBoxElement.AutoCompleteMode = System.Windows.Forms.AutoCompleteMode.SuggestAppend;
                ...
                this.radMultiColumnComboBoxProcesos.DataSource = this.dtUsuarios;

                FilterDescriptor descriptor = new FilterDescriptor(this.radMultiColumnComboBoxProcesos.DisplayMember, FilterOperator.StartsWith, string.Empty);
                this.radMultiColumnComboBoxProcesos.EditorControl.FilterDescriptors.Add(descriptor);
                this.radMultiColumnComboBoxProcesos.DropDownStyle = Telerik.WinControls.RadDropDownStyle.DropDownList;
            }
            catch (Exception ex) { Log.Error(Utiles.CreateExceptionString(ex)); }
        }
